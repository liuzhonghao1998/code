...
func (app *Application) DeliveryHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.FormValue("submitted") == "true" {
		...
		if messageType == "start" {
			...
			// 	return
			// }
			// vote, err = app.Fabric.QueryVoteByBlockNum(num)
			// if err != nil {
			// 	http.Error(w, "Fail to query the vote data", 500)
			// 	return
			// }
			// data.Query = false
			// data.CurrentHeight = votes[0].CurrentHeight
			// data.TxID = votes[0].TxID
			// data.TxNum = "1"
			// data.BlockNum = votes[0].BlockNum
			// data.Candidate = votes[0].Candidate
			// data.HashedGeo = votes[0].HashedGeo
			// data.LinkableRingSign = votes[0].LinkableRingSignature
			// data.Start = true
			// data.Success = true
			// data.Response = false
			// renderTemplate(w, r, "delivery.html", data)
			...
			if len(Reports.Showlist) < 5 {
				Reports.Showlist = append(Reports.Showlist, *vote)
			} else {
				Reports.Showlist = append(Reports.Showlist[1:], *vote)
			}
