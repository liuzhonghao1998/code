...
int match();  // 查找函数 
int Fuzzysearch(string term, SportsP temp); //模糊查找
...
int League::run() {
	...
	char sendBuf[30] = { 0 };//发送至客户端的字符串
	char recvBuf[30] = { 0 };//接受客户端返回的字符串

							  //会阻塞进程，直到有客户端连接上来为止
	serversocket = accept(sockServer, (SOCKADDR*)&addrClient, &len);
	cout << "监听成功" << endl;
	...
	string trem; //查找要素
	string talk, fileS, fileG, NfileS, NfileG;//告诉客户端发送完毕
	int choose, Choose, option, fileNY, CH; //对球员还是行政人员操作，对俱乐部进行操作还是对俱乐部内部进行操作，什么操作，文件操作 ,是否执行服务器内部操作
	int flag; //用于各种标识 
	int gai;//想更改的选项
	...
	while (1) {
		if (fileNY == 1) {    //用文件进行初始化
			...
			cout << "是否执行服务器内部操作(1.是0.不是):";
			cin >> CH;
			getchar();
			if (CH == 0) {//接受外部数据
						  //接收并打印客户端数据
				cout << "开始通信" << endl;
				while (SOCKET_ERROR == recv(serversocket, recvBuf, sizeof(recvBuf)+1, 0)) {
					cout << "接受错误,或链接中断\n" << endl;
					return 0;
				}
				if (recvBuf[0] == '4') {
					...
					for (Cit = lis.begin(); Cit != lis.end(); Cit++) {
						...
						for (Sit = C.lis2.begin(); Sit != C.lis2.end(); Sit++) {
							if (match()) {
								cout << (*Sit).name << "符合条件" << endl;
								int count = 1;
								while (count != 9) {
									switch (count) {
									case 2: //年龄
										itoa((*Sit).age, sendBuf, 10);
										break;
									case 1:   //名字
										strcpy(sendBuf, (*Sit).name.c_str());
										break;
									case 3:  //能力
										strcpy(sendBuf, (*Sit).getstrength().c_str());
										break;
									case 8: //俱乐部
										strcpy(sendBuf, (*Sit).getclub().c_str());
										break;
									case 4: //年薪
									case 5:  //转会费
									case 6: // 进球总数
									case 7: //服役年限
										itoa((*Sit).getmessage(count - 3), sendBuf, 10);
										break;
									}
					...
					talk = "send is over";
					strcpy(sendBuf, talk.c_str());
					send(serversocket, sendBuf, sizeof(sendBuf) + 1, 0);
					memset(behind, 0, sizeof(behind));
				}
				else if (recvBuf[0] == '5') {
					recv(serversocket, recvBuf, sizeof(recvBuf) + 1, 0);
					N = recvBuf;
					for (Cit = lis.begin(); Cit != lis.end(); Cit++) {  //找到对应的俱乐部
						Ctemp = *Cit;
						if (N == (Ctemp.putname())) {
							flag = 0;
							break;
					...
					recv(serversocket, recvBuf, sizeof(recvBuf) + 1, 0);
					n = recvBuf;
					flag = Ctemp.deleteP(n, 1);
					memset(recvBuf, 0, sizeof(recvBuf));
					recv(serversocket, recvBuf, sizeof(recvBuf) + 1, 0);
					Transfer_fee = atoi(recvBuf);
					memset(recvBuf, 0, sizeof(recvBuf));
					a = S.getmessage(2);
					if (Transfer_fee >= a) {
						S.putclub(N);
						...
						while (count != 9) {
							switch (count) {
							case 2: //年龄
								itoa(S.age, sendBuf, 10);
								break;
							case 1:   //名字
								strcpy(sendBuf, S.name.c_str());
								break;
							case 3:  //能力
								strcpy(sendBuf, S.getstrength().c_str());
								break;
							case 8: //俱乐部
								strcpy(sendBuf, S.getclub().c_str());
								break;
							case 4: //年薪
							case 5:  //转会费
							case 6: // 进球总数
							case 7: //服役年限
								itoa(S.getmessage(count - 3), sendBuf, 10);
								break;
							}
							send(serversocket, sendBuf, sizeof(sendBuf) + 1, 0);
							memset(sendBuf, 0, sizeof(sendBuf));
...
int Fuzzysearch(string term, SportsP temp) {
